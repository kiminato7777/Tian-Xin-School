            ...
            Object.values(studentsData).forEach(student => {
                ...
                if (initialPay > 0 && !overrideIds.has(regId)) {
                    transactionsData.push({
                        id: regId,
                        type: 'income',
                        category: 'ការចុះឈ្មោះ (Registration)',
                    ...
                    instObj.forEach((inst, idx) => {
                        ...
                        if (amt > 0 && !overrideIds.has(instId)) {
                            transactionsData.push({
                                id: instId,
                                type: 'income',
                                category: 'បង់ប្រាក់សិក្សា (Tuition Fee)',
                                description: `បង់ប្រាក់ខែ${inst.forMonth ? ` ${inst.forMonth}` : ''}${inst.period ? ` (${inst.period})` : (inst.months ? ` (ចំនួន ${inst.months} ខែ)` : '')}`,
                                amount: amt,
    ...
    paginatedData.forEach((item, index) => {
        ...
        const actionButtons = `
            <div class="d-flex justify-content-center gap-2">
                <button class="btn btn-sm btn-light text-primary shadow-sm rounded-circle me-1" style="width: 32px; height: 32px;" onclick="editTransaction('${item.id}')" title="កែប្រែ">
        ...
        row.innerHTML = `
            ...
            <td class="text-center text-dark fw-bold" style="font-family: 'Khmer OS Battambang', sans-serif;">
            ...
            <td class="text-end pe-4">
                <span class="${amountClass} fw-bold fs-6" style="letter-spacing: 0.5px;">
...
function handleFormSubmit(e) {
    ...
    // Amount Calculation (USD + KHR)
    const amountUSD = parseFloat(document.getElementById('transAmount').value) || 0;
    const amountKHR = parseFloat(document.getElementById('transAmountKHR').value) || 0;
    const exchangeRate = 4100; // 1 USD = 4100 KHR

    const amount = amountUSD + (amountKHR / exchangeRate);
...
function editTransaction(id) {
    ...
    document.getElementById('transAmountKHR').value = ''; // Clear KHR field
