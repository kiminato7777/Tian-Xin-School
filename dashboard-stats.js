        ...
        Object.values(studentsData).forEach(student => {
            ...
            if (student.paymentDueDate) {
                try {
                    ...
                    // Check if upcoming (and has remaining balance)
                    if (dueDate > now && dueDate <= tenDaysFromNow && remaining > 0) {
                        upcomingPayment++;
                    }
                } catch (dateError) {
...
function updateExpenseStats(expenseData) {
    // Function disabled as UI element was replaced
}
...
function loadAllData() {
    ...
    Promise.all([
        ...
        // updateExpenseStats(expenseData); // Disabled
