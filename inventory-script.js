...
async function exportToPDF() {
    ...
    // Load Khmer Font
    try {
        const response = await fetch('fonts/Khmer OS Battambang.ttf');
        ...
        reader.onloadend = function () {
            ...
            // Add Font to jsPDF
            doc.addFileToVFS('KhmerOSBattambang.ttf', base64data);
            doc.addFont('KhmerOSBattambang.ttf', 'KhmerOSBattambang', 'normal');
            doc.setFont('KhmerOSBattambang');
            doc.setFontSize(14);

            // Title
            doc.text("របាយការណ៍ស្តុកសម្ភារៈ (Inventory Report)", 14, 15);

            const type = document.getElementById('exportType').value;
            doc.setFontSize(10);
            const dateStr = new Date().toLocaleDateString('km-KH');
            doc.text(`ប្រភេទ: ${type} - កាលបរិច្ឆេទ: ${dateStr}`, 14, 22);

            // Table Headers (Khmer)
            const headers = [["ល.រ", "អ្នកកាន់ស្តុក", "ឈ្មោះទំនិញ", "កាលបរិច្ឆេទ", "ស្តុកចូល", "ស្តុកចេញ", "ស្តុកនៅសល់", "ចំណាំ"]];

            // Generate Data
            const data = Object.values(inventoryData).map((item, index) => {
                const totalIn = parseInt(item.totalIn) !== undefined ? parseInt(item.totalIn) : (parseInt(item.quantity) || 0);
                const stockOut = parseInt(item.stockOut) || 0;
                const remaining = totalIn - stockOut;

                return [
                    (index + 1).toString(), // Ensure string
                    item.stockKeeper || '-',
                    item.itemName,
                    item.importDate || '-',
                    totalIn.toString(),
            ...
            doc.save(`inventory_report_${type}.pdf`);
        };
        ...
        alert("មិនអាចទាញយកពុម្ពអក្សរភាសាខ្មែរ។ PDF នឹងមិនមានអក្សរខ្មែរទេ។ (Could not load Khmer font)");

        // Fallback
        doc.text("Inventory Report (Error loading Khmer font)", 14, 15);
        doc.save('inventory_report_error.pdf');
    }
}
...
function exportToExcel() {
    ...
    XLSX.writeFile(workbook, `inventory_report_${type}.xlsx`);
}
