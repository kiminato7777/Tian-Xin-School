...
function handleAddInventory(e) {
    ...
    const item = {
        ...
        officeIn: 0,          // Nothing in office yet
...
function openRestockModal(id) {
    ...
    const currentWarehouse = warehouseIn - officeIn;
...
function openTransferModal(id) {
    ...
    const warehouseIn = parseInt(item.warehouseIn) || 0;
    const officeIn = parseInt(item.officeIn) || 0;
    const currentWarehouse = warehouseIn - officeIn;
...
function handleTransfer(e) {
    ...
    const logEntry = `[Transfer ${dateStr}: ${qty}] ${note}`;
...
function renderInventoryTable() {
    ...
    // Render columns: #, Item Name, Warehouse Stock, Office Stock, Sold Out, Total, Stock Keeper, Actions
    items.forEach(([id, item]) => {
        if (searchTerm && !item.itemName.toLowerCase().includes(searchTerm)) return;
        ...
        // Current Balances
        const currentWarehouse = Math.max(0, warehouseIn - officeIn);
...
function updateSummaryCards() {
    ...
    Object.values(inventoryData).forEach(item => {
        ...
        const warehouseIn = parseInt(item.warehouseIn) || parseInt(item.totalIn) || parseInt(item.quantity) || 0;
        const stockOut = parseInt(item.stockOut) || 0;
        const remaining = warehouseIn - stockOut;

        statTotalIn += warehouseIn;
...
async function exportToPDF() {
    ...
    <div class="action-bar no-print">
        <button class="btn btn-home" onclick="window.close()">
            <i class="fas fa-home"></i> ត្រឡប់ទៅផ្ទាំងដើម (Back Home)
        </button>
        <button class="btn btn-print" onclick="window.print()">
            <i class="fas fa-print"></i> បោះពុម្ពរបាយការណ៍ (Print Report)
        </button>
    </div>
    ...
    <table>
        ...
        ${(() => {
            if (category === 'stockOut') {
                ...
                let tWh = 0, tOff = 0, tOut = 0, tTotal = 0, tValUSD = 0;
                filteredData.forEach(item => {
                    const win = parseInt(item.warehouseIn) || parseInt(item.totalIn) || parseInt(item.quantity) || 0;
                    const oin = parseInt(item.officeIn) || 0;
                    const sout = parseInt(item.stockOut) || 0;
                    const cw = Math.max(0, win - oin);
                    const co = Math.max(0, oin - sout);
                    const rem = cw + co;
                    tWh += cw; tOff += co; tOut += sout; tTotal += rem;
                    tValUSD += (rem * (parseFloat(item.sellingPrice) || 0));
                });
                return `
                    <tr style="background-color: #f2f2f2; font-weight: bold; border-top: 2px solid #333;">
                        <td colspan="3" class="text-right">សរុបរួម (Grand Total):</td>
                        ...
                        <td class="price-col text-primary">${Math.round(tValUSD * exchangeRate).toLocaleString()} ៛</td>
                    </tr>`;
            }
        })()}
        </tbody>
    ...
    ${(() => {
            if (category === 'stockOut') {
                ...
                <div class="summary-box">
                    <h4 style="margin: 0 0 15px 0;"><i class="fas fa-chart-line me-2"></i> សង្ខេបការលក់សរុប (Grand Summary)</h4>
                ...
                filteredData.forEach(item => {
                    ...
                    const oin = parseInt(item.officeIn) || 0;
                    const sout = parseInt(item.stockOut) || 0;
                    const cWh = Math.max(0, win - oin);
                    const cOff = Math.max(0, oin - sout);
                    const rem = cWh + cOff;
                ...
                return `
                <div class="summary-box">
                    <h4 style="margin: 0 0 15px 0;"><i class="fas fa-boxes me-2"></i> សង្ខេបតុល្យភាពស្តុក និងទឹកប្រាក់ (Inventory Value Summary)</h4>
