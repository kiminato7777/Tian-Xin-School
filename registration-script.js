...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeCourseSelection() {
        // Course checkboxes
        if (chineseFulltimeCheckbox) {
            chineseFulltimeCheckbox.addEventListener('change', handleCourseSelection);
        }
        if (chineseParttimeCheckbox) {
            chineseParttimeCheckbox.addEventListener('change', handleCourseSelection);
        }
        ...
        if (studyDurationSelect) {
            studyDurationSelect.addEventListener('change', calculateTuitionFee);
        }
    ...
    function handleCourseSelection() {
        if (this.checked) {
            if (this.id === 'reg_cFullTime') {
                ...
                // Show fulltime book option, hide regular book
                ...
                if (fulltimeBookCheckbox) fulltimeBookCheckbox.style.display = 'block';
                if (regularBookCheckbox && regularBookCheckbox.parentElement && regularBookCheckbox.parentElement.parentElement && regularBookCheckbox.parentElement.parentElement.parentElement) {
                    regularBookCheckbox.parentElement.parentElement.parentElement.style.display = 'none';
                }

                // Uncheck part-time duration checkboxes
                document.querySelectorAll('.duration-checkbox').forEach(cb => cb.checked = false);

                updateStudyDurationOptions(true);

                // Reset part-time old student
                const oldStudentCheck = document.getElementById('reg_isOldStudentCheck');
                if (oldStudentCheck) {
                ...
                // Hide part-time manual duration
                if (parttimeManualDurationCheckbox) {
                    parttimeManualDurationCheckbox.checked = false;
                    if (parttimeManualDurationFields) parttimeManualDurationFields.style.display = 'none';
                }

                // Auto-set payment months for manual input
                if (enableManualFeeCheckbox && enableManualFeeCheckbox.checked) {
                    const studyDurationManual = document.getElementById('reg_studyDuration_manual');
                    const paymentMonthsManual = document.getElementById('reg_paymentMonths_manual');
                    if (studyDurationManual && paymentMonthsManual) {
                        const studyDuration = studyDurationManual.value || '6';
                        paymentMonthsManual.value = studyDuration;
                    }
                }

                // ទាញយកតម្លៃសម្រាប់ការបញ្ចូលដោយខ្លួនឯង
                if (fulltimeManualDurationCheckbox && fulltimeManualDurationCheckbox.checked) {
                    ...
                    if (paymentMonthsInput && !paymentMonthsInput.value) {
                        paymentMonthsInput.value = studyDurationInput ? studyDurationInput.value : '6';
                    }
                }

            } else if (this.id === 'reg_cPartTime') {
                ...
                // Hide fulltime book option, show regular book
                const fulltimeBookCheckbox = document.querySelector('.fulltime-book-checkbox');
                const regularBookCheckbox = document.getElementById('reg_bookCheck');
                ...
                const fulltimeBookCheck = document.getElementById('reg_fulltimeBookCheck');
                if (fulltimeBookCheck) fulltimeBookCheck.checked = false;

                updateStudyDurationOptions(false);

                // Hide full-time manual duration
                if (fulltimeManualDurationCheckbox) {
                ...
                if (enableManualFeeCheckbox && enableManualFeeCheckbox.checked) {
                    ...
                    const paymentMonthsManual = document.getElementById('reg_paymentMonths_manual');
                    if (studyDurationManual && paymentMonthsManual) {
                        const studyDuration = studyDurationManual.value || '3';
                        paymentMonthsManual.value = studyDuration;
                    }
                }
            }
    ...
    function calculateFees() {
        ...
        try {
            ...
            if (enableManualFeeCheckbox && enableManualFeeCheckbox.checked) {
                ...
                const totalAdminFeesField = document.getElementById('reg_totalAdminFees');
                if (totalAdminFeesField) {
                    totalAdminFeesField.value = (materialFee + adminFee + adminServicesFee).toFixed(2);
                }
                ...
                bookFee = bookFeeField ? parseFloat(bookFeeField.value) || 0 : 0;
                fulltimeBookFee = fulltimeBookFeeField ? parseFloat(fulltimeBookFeeField.value) || 0 : 0;
                uniformFee = uniformFeeField ? parseFloat(uniformFeeField.value) || 0 : 0;
                idCardFee = idCardFeeField ? parseFloat(idCardFeeField.value) || 0 : 0;
                registrationFee = registrationFeeField ? parseFloat(registrationFeeField.value) || 0 : 0;
            ...
            if (enableManualFeeCheckbox && enableManualFeeCheckbox.checked) {
                totalAdminFees = materialFee + adminFee + adminServicesFee;
            } else {
                const totalAdminFeesField = document.getElementById('reg_totalAdminFees');
                totalAdminFees = totalAdminFeesField ? parseFloat(totalAdminFeesField.value) || 0 : 0;
                totalAdminFees += adminServicesFee; // បន្ថែមសេវារដ្ឋបាល
            }

            const totalAllFees = netTuition + totalAdminFees;
            const balance = Math.max(0, totalAllFees - initialPayment);

            if (!enableManualFeeCheckbox.checked) {
                const totalAdminFeesField = document.getElementById('reg_totalAdminFees');
                if (totalAdminFeesField) {
                    totalAdminFeesField.value = totalAdminFees.toFixed(2);
                }
            ...
            // Update summary - បង្ហាញថ្លៃសម្ភារៈដាច់ដោយឡែក
            updateSummaryDisplay(tuitionFee, discount, discountPercent, totalDiscount, netTuition,
                materialFee, adminFee, adminServicesFee, totalAdminFees, totalAllFees, initialPayment, balance,
                bookFee, fulltimeBookFee, uniformFee, idCardFee, registrationFee);
    ...
    function updateSummaryDisplay(tuitionFee, discount, discountPercent, totalDiscount, netTuition,
        ...
        // Update admin materials section
        if (summaryAdminMaterials) summaryAdminMaterials.textContent = `$${totalAdminFees.toFixed(2)}`;

        // Update individual material fees
        if (summaryBookFee) summaryBookFee.textContent = `$${bookFee.toFixed(2)}`;
        if (summaryFulltimeBookFee) summaryFulltimeBookFee.textContent = `$${fulltimeBookFee.toFixed(2)}`;
        if (summaryUniformFee) summaryUniformFee.textContent = `$${uniformFee.toFixed(2)}`;
        if (summaryIdCardFee) summaryIdCardFee.textContent = `$${idCardFee.toFixed(2)}`;
        if (summaryRegistrationFee) summaryRegistrationFee.textContent = `$${registrationFee.toFixed(2)}`;
        if (summaryAdminFee) summaryAdminFee.textContent = `$${adminFee.toFixed(2)}`;
        if (summaryServicesFee) summaryServicesFee.textContent = `$${adminServicesFee.toFixed(2)}`;
        if (summaryTotalAdminFees) summaryTotalAdminFees.textContent = `$${totalAdminFees.toFixed(2)}`;
        ...
        // Show/hide fulltime book fee details
        const fulltimeBookFeeItem = document.querySelector('.fee-sub-item:nth-child(2)');
        if (fulltimeBookFeeItem) {
            if (chineseFulltimeCheckbox && chineseFulltimeCheckbox.checked) {
                fulltimeBookFeeItem.style.display = 'flex';
            } else {
                fulltimeBookFeeItem.style.display = 'none';
            }
        }

        // Show/hide services fee
        if (summaryServicesFeeLabel && summaryServicesFeeLabel.parentElement) {
            if (adminServicesFee > 0 || (enableManualFeeCheckbox && enableManualFeeCheckbox.checked)) {
                summaryServicesFeeLabel.parentElement.style.display = 'flex';
            } else {
                summaryServicesFeeLabel.parentElement.style.display = 'none';
            }
    ...
    function calculateAdminMaterials() {
        if (isCalculatingAdminMaterials) return;
        isCalculatingAdminMaterials = true;
        ...
        try {
            ...
            checkboxes.forEach(checkbox => {
                if (checkbox.checked) {
                    ...
                    const inputId = checkbox.id.replace('Check', 'Fee');
            ...
            // បន្ថែមសេវារដ្ឋបាល
            if (adminServicesCheckbox && adminServicesCheckbox.checked && adminServicesFeeInput) {
                total += parseFloat(adminServicesFeeInput.value) || 0;
            }

            const totalAdminFeesField = document.getElementById('reg_totalAdminFees');
            if (totalAdminFeesField) {
                totalAdminFeesField.value = total.toFixed(2);
            }
        } finally {
            isCalculatingAdminMaterials = false;
        }
    }
    ...
    function setupInstallmentSystem() {
        ...
        if (enableInstallmentCheckbox) {
            enableInstallmentCheckbox.addEventListener('change', function () {
                ...
                if (this.checked) {
                    if (installmentStages) installmentStages.style.display = 'block';
                    updateInstallmentStages();
                } else {
                    if (installmentStages) installmentStages.style.display = 'none';
                }
            });
        }
    }
    ...
    function validateForm() {
        ...
        if (chineseFulltimeCheckbox && chineseFulltimeCheckbox.checked) {
            ...
            if (fulltimeManualDurationCheckbox && fulltimeManualDurationCheckbox.checked) {
                ...
                if (studyDuration <= 0 || studyDuration > 120) { // 10 ឆ្នាំ = 120 ខែ
                    showAlert('រយៈពេលសិក្សាត្រូវតែចន្លោះ 1-120 ខែ (10 ឆ្នាំ)', 'warning');
                ...
                const paymentMonthsManual = document.getElementById('reg_paymentMonths_manual');
                const paymentMonths = paymentMonthsManual ? parseInt(paymentMonthsManual.value) || 0 : 0;
                if (paymentMonths <= 0 || paymentMonths > 120) {
                    showAlert('រយៈពេលបង់ត្រូវតែចន្លោះ 1-120 ខែ (10 ឆ្នាំ)', 'warning');
        ...
        if (chineseParttimeCheckbox && chineseParttimeCheckbox.checked) {
            ...
            if (parttimeManualDurationCheckbox && parttimeManualDurationCheckbox.checked) {
                const studyDurationManual = document.getElementById('reg_parttime_studyDuration_manual');
                const studyDuration = studyDurationManual ? parseInt(studyDurationManual.value) || 0 : 0;
                if (studyDuration <= 0 || studyDuration > 120) {
                    showAlert('រយៈពេលសិក្សាត្រូវតែចន្លោះ 1-120 ខែ (10 ឆ្នាំ)', 'warning');
                    if (studyDurationManual) studyDurationManual.focus();
                    return false;
                }
        ...
        if (enableManualFeeCheckbox && enableManualFeeCheckbox.checked) {
            ...
            if (parseInt(paymentMonths) > 120) {
                showAlert('រយៈពេលបង់មិនត្រូវលើស 120 ខែ (10 ឆ្នាំ)', 'warning');
                if (paymentMonthsManual) paymentMonthsManual.focus();
                return false;
            }
        }
    ...
    function collectFormData() {
        ...
        const studyLevel = document.querySelector('input[name="reg_studyLevel"]:checked')?.value || '';
    ...
    function populateFormForEdit(data) {
        ...
        if (data.courseType === 'chinese-fulltime') {
            if (chineseFulltimeCheckbox) {
                chineseFulltimeCheckbox.checked = true;
                handleCourseSelection.call(chineseFulltimeCheckbox);
            }
        } else if (data.courseType === 'chinese-parttime') {
            if (chineseParttimeCheckbox) {
                chineseParttimeCheckbox.checked = true;
                handleCourseSelection.call(chineseParttimeCheckbox);
            }
        }
        ...
        if (data.enableInstallment) {
            ...
            if (installmentCheck) {
                ...
                if (document.getElementById('installmentStages')) document.getElementById('installmentStages').style.display = 'block';
            }
    ...
    function setupEventListeners() {
        ...
        // Admin materials checkboxes
        document.querySelectorAll('.material-checkbox').forEach(checkbox => {
            checkbox.addEventListener('change', calculateAdminMaterials);
        });

        // Admin materials input fields
        const adminFeeInputs = ['reg_bookFee', 'reg_fulltimeBookFee', 'reg_uniformFee', 'reg_idCardFee', 'reg_registrationFee', 'reg_adminFee'];
        adminFeeInputs.forEach(id => {
            ...
            if (input) {
                ...
                input.addEventListener('input', function () {
                    ...
                    timeout = setTimeout(() => calculateFees(), 300);
                });
            }
        });
