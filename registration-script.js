...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeCourseSelection() {
        ...
        const threeLanguagesCheckbox = document.getElementById('reg_cThreeLanguages');
        if (threeLanguagesCheckbox) {
            threeLanguagesCheckbox.addEventListener('change', handleCourseSelection);
        }
    ...
    function handleCourseSelection() {
        if (this.checked) {
            if (this.id === 'reg_cFullTime') {
                ...
                const threeLanguagesCheckbox = document.getElementById('reg_cThreeLanguages');
                if (threeLanguagesCheckbox) threeLanguagesCheckbox.checked = false;
                ...
                // Restore sections hidden by 3-Language
                ...
                if (topTeacherCard) {
                    topTeacherCard.style.display = 'block';
                    if (topTeacherCard.previousElementSibling && topTeacherCard.previousElementSibling.classList.contains('section-title')) {
                        topTeacherCard.previousElementSibling.style.display = 'block';
                    }
                }
                ...
                // Hide manual payment months input, show select
                ...
                if (enableManualFeeCheckbox && enableManualFeeCheckbox.checked) {
                    const studyDurationManual = document.getElementById('reg_studyDuration_manual');
                    // const paymentMonthsManual = document.getElementById('reg_paymentMonths_manual'); // OLD ID
            ...
            } else if (this.id === 'reg_cThreeLanguages') {
                if (chineseFulltimeCheckbox) chineseFulltimeCheckbox.checked = false;
                if (chineseParttimeCheckbox) chineseParttimeCheckbox.checked = false;
                ...
                // Hide sections for 3-Language
                const teacherInfo = document.getElementById('teacherInfoSection');
                if (teacherInfo) teacherInfo.style.display = 'none';

                const generalEdu = document.getElementById('generalEducationSection');
                if (generalEdu) generalEdu.style.display = 'none';
                ...
                const topTeacherCard = document.querySelector('.teacher-card');
                if (topTeacherCard) {
                    topTeacherCard.style.display = 'none';
                    if (topTeacherCard.previousElementSibling && topTeacherCard.previousElementSibling.classList.contains('section-title')) {
                        topTeacherCard.previousElementSibling.style.display = 'none';
                    }
                }
                ...
                // Show manual payment months input, hide select
                const paymentMonthsSelect = document.getElementById('reg_paymentMonthsSelect');
                const paymentMonthsManual = document.getElementById('reg_paymentMonthsManualInput');
                ...
                if (paymentMonthsManual) {
                    ...
                    paymentMonthsManual.value = '36'; // Default to 36 as requested
                }
            }
        } else {
            if (chineseFulltimeDetails) chineseFulltimeDetails.style.display = 'none';
            ...
            if (this.id === 'reg_cThreeLanguages') {
                const threeLanDetails = document.getElementById('threeLanguagesDetails');
                if (threeLanDetails) threeLanDetails.style.display = 'none';
    ...
    function calculateFees() {
        ...
        try {
            ...
            // Total fees is just net tuition now (since admin fees removed)
            const totalAllFees = netTuition;
            ...
            // Update summary
            updateSummaryDisplay(tuitionFee, discount, discountPercent, totalDiscount, netTuition, totalAllFees, initialPayment, balance);
    ...
    function updateSummaryDisplay(tuitionFee, discount, discountPercent, totalDiscount, netTuition, totalAllFees, initialPayment, balance) {
    ...
    function calculateAdminMaterials() {
        ...
        try {
            ...
            // Fulltime Book Only
            const fulltimeBookCheck = document.getElementById('reg_fulltimeBookCheck');
            if (fulltimeBookCheck && fulltimeBookCheck.checked) {
                const val = parseFloat(fulltimeBookCheck.value) || 0;
                total += val;
                document.getElementById('reg_fulltimeBookFee').value = val.toFixed(2);
            } else if (document.getElementById('reg_fulltimeBookFee')) {
                document.getElementById('reg_fulltimeBookFee').value = '0.00';
            }

            // Admin Services Fee Logic
            const adminCheck = document.getElementById('reg_adminCheck');
            const adminMonthsSelect = document.getElementById('reg_adminServicesMonths');
            const adminFeeInput = document.getElementById('reg_adminFee');

            if (adminCheck && adminCheck.checked) {
                const months = adminMonthsSelect ? parseInt(adminMonthsSelect.value) || 1 : 1;
                const fee = 25.00 * months;
                if (adminFeeInput) adminFeeInput.value = fee.toFixed(2);
                total += fee;
            } else {
                if (adminFeeInput) adminFeeInput.value = '0.00';
            }
